...
function TaskRow(props) {
  ...
  const getRepetitionFrequencyString = task => {
    ...
    } if (task.repeatUnit === 1) {
      if (task.repeatInterval === 1) {
        return ' weekly';
      }
      return ` every ${task.repeatInterval} weeks`;
    } if (task.repeatUnit === 2) {
      return ' monthly';
    }
  };
  ...
  return (
    <div className="task" onClick={() => { console.log(task); updateTask(task); }}>
        ...
        && (
          <>
            ...
            <div className="helper-text">
              Resets
              {getRepetitionFrequencyString(task)}
            </div>
            {
